...
describe('keys(), values(), entries()', function() {
    ...
    const descriptors = [
        ...
        [ Symbol('s'), { value: 3, enumerable: false, configurable: true, writable: true } ]
    ];

    before(function() {
        const descriptorMap = descriptors.reduce(
            (acc, [ key, descriptor ]) => {
                acc[key] = descriptor;
                return acc;
            },
    ...
    describe('keys()', function() {
        ...
        describe('keys.enum()', function() {
            it('Should return only enumerable keys', function() {
                ...
                const enumKeys = descriptors.filter(([, descriptor ]) => descriptor.enumerable).map(([key]) => key);
        ...
        describe('keys.names()', function() {
            it('Should return all string keys', function() {
                ...
                const stringKeys = descriptors.filter(([ key ]) => $STRING(key)).map(([key]) => key);
        ...
        describe('keys.symbols()', function() {
            it('Should return all symbol keys', function() {
                ...
                const symbolKeys = descriptors.filter(([ key ]) => $SYMBOL(key)).map(([key]) => key);
    ...
    describe('values()', function() {
        ...
        describe('values.enum()', function() {
            it('Should return only values for enumerable properties', function() {
                ...
                const enumValues = descriptors.filter(([, descriptor ]) => descriptor.enumerable).map(([, descriptor]) => descriptor.value);
        ...
        describe('values.names()', function() {
            it('Should return values for all string properties', function() {
                ...
                const stringValues = descriptors.filter(([ key ]) => $STRING(key)).map(([, descriptor]) => descriptor.value);
        ...
        describe('values.symbols()', function() {
            it('Should return values for all symbol properties', function() {
                ...
                const symbolValues = descriptors.filter(([ key ]) => $SYMBOL(key)).map(([, descriptor]) => descriptor.value);
    ...
    describe('entries()', function() {
        ...
        describe('entries.enum()', function() {
            it('Should return only entries for enumerable properties', function() {
                ...
                const enumEntries = descriptors.filter(([, descriptor ]) => descriptor.enumerable).map(([ key, descriptor ]) => [ key, descriptor.value ]);
        ...
        describe('entries.names()', function() {
            it('Should return entries for all string properties', function() {
                ...
                const stringEntries = descriptors.filter(([ key ]) => $STRING(key)).map(([ key, descriptor]) => [ key, descriptor.value ]);
        ...
        describe('entries.symbols()', function() {
            it('Should return entries for all symbol properties', function() {
                ...
                const symbolEntries = descriptors.filter(([ key ]) => $SYMBOL(key)).map(([ key, descriptor]) => [ key, descriptor.value ]);
